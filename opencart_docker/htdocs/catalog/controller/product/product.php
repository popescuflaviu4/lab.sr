...
require_once(DIR_SYSTEM . '../system/helper/recombee.php');
...
class Product extends \Opencart\System\Engine\Controller {
	...
	public function index(): \Opencart\System\Engine\Action|null {
		...
		if ($product_info) {
			...
			$recombee = new \RecombeeHelper();
			// $recommendations = $recombee->getRecommendations(
			// 	$this->customer->isLogged() ? $this->customer->getId() : session_id()
			// );
			$recommendations = $recombee->getRecommendations($this->session->getId());

			$data['recommended_products'] = [];
			foreach ($recommendations as $recommendation) {
				$product_info = $this->model_catalog_product->getProduct($recommendation['id']);
				if ($product_info) {
					$data['recommended_products'][] = [
